...
ins_alerts AS (
  ...
  VALUES
    ((SELECT id FROM co), 'maintenance', 'high',   'PM Overdue: T-101',                                          'Oil change is overdue by 340 miles. Schedule service immediately.', (CURRENT_DATE + (0  || ' days')::interval)::date, FALSE, 'truck',     'T-101',         TRUE,  'maintenance:pm_overdue:T-101',                NULL),
    ((SELECT id FROM co), 'safety',      'medium', 'Driver Safety Flag: Devon Carter',                           '2 speeding events in the last 7 days. Coaching session recommended.', (CURRENT_DATE + (-1 || ' days')::interval)::date, FALSE, 'driver',    'Devon Carter',  TRUE,  'safety:driver_flag:Devon Carter',             NULL),
    ((SELECT id FROM co), 'compliance',  'medium', 'CDL Renewal Approaching: Devon Carter',                      'CDL expires Dec 1, 2025 (216 days). Begin renewal process.',          (CURRENT_DATE + (-2 || ' days')::interval)::date, FALSE, 'driver',    'Devon Carter',  TRUE,  'compliance:cdl_expiring:Devon Carter',        NULL),
    ((SELECT id FROM co), 'insurance',   'low',    'Insurance Renewal Process Started',                          'Progressive renewal due May 11. Multi-broker quote initiated.',       (CURRENT_DATE + (-3 || ' days')::interval)::date, TRUE,  'insurance', 'PC-44218903',   FALSE, NULL,                                          NULL),
    ((SELECT id FROM co), 'compliance',  'medium', 'Med Card Expiring: Brandon Kim',                             'Medical card expires Aug 5, 2025. Schedule DOT physical.',            (CURRENT_DATE + (-5 || ' days')::interval)::date, TRUE,  'driver',    'Brandon Kim',   TRUE,  'compliance:med_card_expiring:Brandon Kim',    NULL),
